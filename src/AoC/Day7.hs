...
entryToInvPair :: Entry -> (Bag, [NumBag])
entryToInvPair (Entry a b) = (a, b)

childPairs :: [Entry] -> Map Bag [NumBag]
childPairs = fromList . fmap entryToInvPair

offsprings :: NumBag -> Map Bag [NumBag] -> Int
offsprings (multi, b) mp =
  let current = findWithDefault [] b mp
      currentSum = sum $ fmap fst current
      offsprings' = sum $ fmap (`offsprings` mp) current
   in multi * (currentSum + offsprings')

sndStar :: Input -> Output
sndStar = offsprings (1, elfBag) . childPairs
